...
def create_task(
    ...
    _validate_task_dates(payload.start_date, payload.deadline)
...
def update_task_status(
    ...
    if current_user.role == "employee":
        if task.assigned_to != current_user.id and not is_tester:
            raise HTTPException(status_code=403, detail="You can only update tasks assigned to you for work or testing")
        allowed = {"status", "progress_percent", "testing_status"}
        requested = set(payload.model_dump(exclude_unset=True))
        if not requested.issubset(allowed):
            raise HTTPException(status_code=403, detail="Employees can only update task progress or workflow status")
...
def _validate_task_dates(start_date: Optional[date], deadline: Optional[date]) -> None:
    """Reject invalid schedules consistently for task creation and updates."""
    if start_date and deadline and deadline < start_date:
        raise HTTPException(status_code=400, detail="Deadline cannot be before the start date")
...
def update_task(
    ...
    if current_user.role == "employee":
        ...
        if task.assigned_to != current_user.id or not set(changes).issubset({"status", "progress_percent"}):
            raise HTTPException(status_code=403, detail="Employees can only update progress or status on their assigned tasks")
    elif current_user.role not in {"admin", "manager"}:
        raise HTTPException(status_code=403, detail="Not authorized to update this task")
    ...
    _validate_task_dates(
        changes.get("start_date", task.start_date),
        changes.get("deadline", task.deadline),
    )
    ...
    if "progress_percent" in changes and not 0 <= changes["progress_percent"] <= 100:
        raise HTTPException(status_code=400, detail="Progress must be between 0 and 100")
    previous_status = task.status
