"""
projects.py
Creates and lists projects, and connects roles through project_team_members:
  - Admin creates a project and assigns managers (+ optional employees)
  - Manager sees only projects they are assigned to
  - Employee sees projects they are on (for documents / context)
  - Client sees projects via client_dashboard (client_id match)
"""
...
def create_project(
    ...
    # Deduplicate so creator + team_user_ids never double-insert the same row
    member_ids = set(team_ids)
    ...
    # Route through ensure_project_member (not raw db.add) so membership
    # inserts get the same duplicate/row-safety guarantees as elsewhere in the app.
